    ...
    <div class="container">
        ...
        @if (Session::has('post_created'))
            <div class="alert alert-primary">
                <span>The following post is created: {{ session('post_created') }}</span>
            </div>
        @endif
        ...
        --}}
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-10">
                    <textarea rows="5" class="form-control @error('text') is-invalid @enderror" id="text" name="text">
                        {{ old('text') }}
                    </textarea>
                    @error('text')
                        <div class="invalid-feedback">
                                ...
                                @foreach ($chunk as $category)
                                    <div class="form-check">
                                        ...
                                        <label for="{{ $category }}" class="form-check-label">
                                            <span
                                                class="badge text-bg-{{ $category->style }}">{{ $category->name }}</span>
                                        </label>
                                        @if ($loop->last)
                                            @error('categories.*')
                                                <div class="invalid-feedback">
