                            ...
                            @foreach ($chunk as $category)
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" value="{{ $category->id }}"
                                        id="{{ $category->id }}" name="categories[]" @checked(in_array($category->id, old('categories', $post->categories->pluck('id')->toArray())))>
                                    {{-- TODO --}}
                                    <label for="{{ $category }}" class="form-check-label">
                                        <span class="badge text-bg-{{ $category->style }}">{{ $category->name }}</span>
                                    </label>
                                    @if ($loop->last)
                                        @error('categories.*')
                                            <div class="invalid-feedback">
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div id="cover_preview" class="col-12">
                                ...
                                <img id="cover_preview_image" src="{{ asset( $post->cover_image_path != "" ? 'storage/'.$post->cover_image_path : 'images/default_post_cover.jpg') }}"
                                    alt="Cover preview">
                            </div>
                        </div>
                    </div>
